...
import br.com.tcc.sctd.dto.ClienteDto;
...
public class RelatorioController {
    ...
    public RelatorioController(FaturaDao faturas, JasperMaker jasperMaker, Result result, VendaDao vendas, ItemVendaDao itensVenda,
            ItemPedidoDao itensPedido, PedidoDao pedidos, ParcelaDao parcelas, ProdutoDao produtos, FuncionarioDao funcionarios, ClienteDao clientes) {
    ...
    @Path("/funcionarios")
    public Download funcionarios() throws Exception {
    ...
    public Download gerarRelatorioProdutosMaisVendidos(Date dataInicial, Date dataFinal) throws DaoException {
        ...
        for(Produto p : listProduto){
            if(mapTipo.isEmpty()){
                ...
                if(p.getModelo().getSexo() != null){
                    if(p.getModelo().getSexo().equals('M')) masculino += p.getQuantidade();
                    else if(p.getModelo().getSexo().equals('F')) feminino += p.getQuantidade();
                    else unisex += p.getQuantidade();
                }
                if(p.getModelo().getTamanho() != null){
                    if(p.getModelo().getTamanho().equals('P')) pequeno += p.getQuantidade();
                    if(p.getModelo().getTamanho().equals('M')) medio += p.getQuantidade();
                    if(p.getModelo().getTamanho().equals('G')) grande += p.getQuantidade();
                }
            }
            else{
        ...
        if(quant.size() > 5) quantidade.addAll(quant.subList(0, 5));
        else quantidade.addAll(quant);
 ...
 */
        for(Parcela p : parcelasList){
            if(p.getDataPagamento() != null){
                ...
                if(p.getDataVencimento().after(ant4Mes) && p.getDataVencimento().before(ant3Mes)){
                    pag1 += 1;
                    if(p.getDataPagamento().after(p.getDataVencimento())) atraso3 ++;
                }
                else if(p.getDataPagamento().after(ant3Mes) && p.getDataPagamento().before(ant2Mes)){
                    pag2 ++;
                    if(p.getDataPagamento().after(p.getDataVencimento())) atraso2 ++;
                }
                else if(p.getDataPagamento().after(ant1Mes) && p.getDataPagamento().before(ant1Mes)){
                    pag1 ++;
                    if(p.getDataPagamento().after(p.getDataVencimento())) atraso1 ++;
                }
        ...
        if(atraso1 > 0) perc1 = (atraso1 * 100) / pag1;
        if(atraso2 > 0)perc2 = (atraso2 * 100) / pag2;
        if(atraso3 > 0)perc3 = (atraso3 * 100) / pag3;
        if(atrasoTotal > 0)percTotal = (atrasoTotal * 100) / pagTotal;
    ...
    @Path("/clientes")
    public void formularioRelatorioClientes() {
        result.include("data", new Date());
    }

    @Path("/clientes/gerar")
    public Download gerarClientes(Date dataInicial, Date dataFinal) throws DaoException {
        System.out.println("gerarClientes gerarClientes gerarClientes gerarClientes gerarClientes");
        
        List<Pedido> pedidosList = pedidos.buscarTodos();
        List<Cliente> clientesList = clientes.buscarTodos();
        Map<String, Object> mapa = new HashMap<String, Object>();
        
        Integer total = 0;
        Integer ativos = 0;
        Integer inadimplentes = 0;
        Integer inativos = 0;
        
        HashMap<String, ClienteDto> clientes = new HashMap<String, ClienteDto>();
        
        for(Pedido p : pedidosList){
            if(p.getDataPedido().after(dataInicial) && p.getDataPedido().before(dataFinal)){
                if(clientes.containsKey(p.getCliente().getNome())){
                    ClienteDto dto = clientes.get(p.getCliente().getNome());
                    Integer q = dto.getQuantidade();
                    BigDecimal v = dto.getValor();

                    for(ItemPedido i : p.getItens()){
                        q += i.getQuantidade();
                        v = v.add(i.getProduto().getValor().multiply(new BigDecimal(i.getQuantidade())));
                    }

                    dto.setQuantidade(q);
                    dto.setValor(v);

                    clientes.put(p.getCliente().getNome(), dto);
                }
                else{
                    ClienteDto dto = new ClienteDto(p.getCliente().getNome(), 0, new BigDecimal(0));
                    Integer q = dto.getQuantidade();
                    BigDecimal v = dto.getValor();
        ...
        total = clientesList.size();
        for(Cliente c : clientesList){
            if(c.getStatus().equals(StatusCliente.ATIVO)) ativos ++;
            if(c.getStatus().equals(StatusCliente.INADIMPLENTE)) inadimplentes ++;
            if(c.getStatus().equals(StatusCliente.DESATIVADO)) inativos ++;
        }
        
        mapa.put("clientes", total);
        mapa.put("ativos", ativos);
        mapa.put("inadimplentes", inadimplentes);
        mapa.put("inativos", inativos);
        mapa.put("dataInicial", dataInicial);
        mapa.put("dataFinal", dataFinal);
        
        ArrayList<ClienteDto> clientesDtoList = new ArrayList<ClienteDto>(clientes.values());
        
        Collections.sort(clientesDtoList, new Comparator() {
            @Override
            public int compare(Object p1, Object p2) {
                ClienteDto tipo1 = (ClienteDto) p1;
                ClienteDto tipo2 = (ClienteDto) p2;
                return tipo1.getQuantidade() < tipo2.getQuantidade() ? 1 : (tipo1.getQuantidade() > tipo2.getQuantidade() ? -1 : 0);
            }
        });
        
        return jasperMaker.makePdf("Clientes.jasper", clientesDtoList, "clientes.pdf", false, mapa);
    }
    ...
    public Download gerarEstoques() throws DaoException {
        ...
        return jasperMaker.makePdf("EstoqueDeProdutos.jasper", produtoListFinal, "estoque.pdf", false, mapa);
    }
    ...
    private static String formatMounthExtensive(Date date) throws Exception{
        ...
        else if(mes.equals("02")) mes = "Fevereiro";
        else if(mes.equals("03")) mes = "Março";
        else if(mes.equals("04")) mes = "Abril";
        else if(mes.equals("05")) mes = "Maio";
        else if(mes.equals("06")) mes = "Junho";
        else if(mes.equals("07")) mes = "Julho";
        else if(mes.equals("08")) mes = "Agosto";
        else if(mes.equals("09")) mes = "Setembro";
        else if(mes.equals("10")) mes = "Outubro";
        else if(mes.equals("11")) mes = "Novembro";
        else if(mes.equals("12")) mes = "Dezembro";
        return  mes;
    }
}
