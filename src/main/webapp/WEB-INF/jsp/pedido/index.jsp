<section id="content" class="row">
    <section class="featured">
        <section class="breadcrumb">
            <span class="parents">
                <a href="<c:url value="/"/>">Página Inicial</a> > 
                <a href="<c:url value="/pedidos/"/>">Pedidos</a> > 
            </span>
            <span class="children">Opções de Pedido</span>
        </section>
        <section class="text-box">
            <fieldset>
                <ul>
                    <li><a href="<c:url value="/pedidos/venda"/>">Registrar Venda</a></li>
                    <li><a href="<c:url value="/pedidos/pedido"/>">Registrar Pedido</a></li>
                    <li><a href="<c:url value="/pedidos/pedido/listar"/>">Listar Pedidos</a></li>
                    <li><a href="<c:url value="/pedidos/pedido/cancelar"/>">Cancelar Pedido</a></li>
                </ul>
                <hr/>
                <ul>
                    <li><a href="<c:url value="/pedidos/ordem/"/>">Ordens de Serviço</a></li>
                    <li><a href="<c:url value="/pedidos/ordem/incluirvenda"/>">Registrar Ordem Serviço - VENDA</a></li>
                    <li><a href="<c:url value="/pedidos/ordem/incluirpedido"/>">Registrar Ordem Serviço - PEDIDO</a></li>
                    <li><a href="<c:url value="/pedidos/ordem/baixar"/>">Baixar Ordem Serviço</a></li>
                </ul>
                <hr/>
                <ul>
                    <li><a href="<c:url value="/pedidos/entrega/recolhimento"/>">Registrar Recolhimento</a></li>
                    <li><a href="<c:url value="/pedidos/entrega/entrega"/>">Registrar Entrega</a></li>
                </ul>
            </fieldset>
        </section>
    </section>
</section>